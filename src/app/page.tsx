...
import { 
  ...
  MapPin, Plus, Mail, Users, Database, 
  LogOut, ChevronLeft, ArrowRight, RefreshCw, 
  Loader2, Key, Trash2, ShieldCheck, Ticket, 
  X, PenTool, Eraser, FileClock, 
  RotateCcw, BookOpen, Info, Search, UserPlus
} from 'lucide-react';
...
interface Order {
  ...
  // 🔹 NUEVOS CAMPOS (necesarios para el filtro)
  state?: string;
  country?: string;
...
function LoadingScreen() { 
  return (
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center text-white p-8">
      ...
      <p className="font-black uppercase text-[10px] tracking-widest opacity-30">Dispatcher Pro...</p>
    </div>
  ); 
}
...
function SuperAdminView({ onLogout }: any) {
    ...
    useEffect(() => onSnapshot(query(collection(db, 'pharmacyEmployees'), where('role','==','license_code')), (s: QuerySnapshot<DocumentData>) => setLicenses(s.docs.map(d=>({id:d.id,...d.data()})))), []);
    ...
    return (<div className="min-h-screen bg-[#0f172a] text-white p-8 flex flex-col"><header className="flex justify-between items-center mb-10"><div><h2 className="text-3xl font-black italic uppercase">Master Hub</h2><p className="text-[10px] text-emerald-500 font-bold uppercase opacity-60">Global Licenses</p></div><button onClick={onLogout} className="p-4 bg-white/10 rounded-2xl"><LogOut/></button></header><div className="space-y-6 flex-1 overflow-y-auto"><div className="bg-white/5 p-8 rounded-4xl border border-white/10 space-y-4 shadow-2xl"><h3 className="font-bold flex gap-2 text-white text-sm uppercase"><Ticket className="text-emerald-500"/> Create License Key</h3><input className="w-full bg-slate-800 p-4 rounded-2xl outline-none text-sm font-bold uppercase" placeholder="Client email" value={email} onChange={v=>setEmail(v.target.value)}/><button onClick={gen} className="w-full bg-emerald-600 text-slate-950 py-5 rounded-4xl font-black uppercase text-xs">Generate Key</button></div><div className="space-y-3">{licenses.map((x: any)=>(<div key={x.id} className="bg-white/5 p-5 rounded-3xl flex justify-between border border-white/5 items-center"><div><p className="font-black text-emerald-400 tracking-wider text-lg">{x.code}</p><p className="text-[10px] opacity-40 uppercase">{x.assignedEmail}</p></div><span className={`text-[8px] px-3 py-1 rounded-full uppercase font-black ${x.status === 'active' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'}`}>{x.status}</span></div>))}</div></div></div>);
}

function StaffManager({ staff }: { staff: Employee[] }) {
  return (
    <div className="space-y-6 animate-in fade-in text-slate-900"> 
      <div className="space-y-2"><p className="text-[10px] font-black text-slate-400 uppercase ml-2">Registered Staff</p>
...
function HistoryLogView({ orders, onBack }: { orders: Order[], onBack: () => void }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col text-slate-900">
        <div className="p-6 flex justify-between items-center bg-white border-b sticky top-0 z-20"><h2 className="text-xl font-black italic uppercase">System Logs</h2><button onClick={onBack} className="bg-slate-100 p-2 rounded-xl text-xs font-bold text-slate-500">Back</button></div>
        <div className="flex-1 overflow-y-auto p-4 space-y-4 pb-24">
            {hist.length === 0 ? <p className="text-center py-20 opacity-30 uppercase text-xs font-black">History empty</p> : hist.map((o: Order) => (
                <div key={o.id} className="bg-white p-5 rounded-3xl shadow-sm border border-slate-100 animate-in"><div className="flex justify-between items-start mb-2"><span className="text-[10px] font-black text-indigo-600 uppercase tracking-widest">#{o.orderCode}</span><span className={`text-[8px] px-2 py-1 rounded font-black uppercase ${o.status === 'delivered' ? 'bg-emerald-100 text-emerald-600' : 'bg-red-100 text-red-600'}`}>{o.status}</span></div><h4 className="font-bold text-slate-800">{o.clientName}</h4><p className="text-[10px] text-slate-400 mb-3 uppercase">{o.address}</p>{o.signatureClient && (<div className="mt-2 border-t border-slate-100 pt-2 text-center"><p className="text-[8px] font-black text-slate-300 uppercase mb-1">Receiver Signature</p><img src={o.signatureClient} alt="Sig" className="h-8 mx-auto object-contain opacity-60" /></div>)}</div>
            ))}
        </div>
    </div>
  );
...
function PharmacyAdminView({ orders, pumps, clients, staff, onLogout, user, pharmacyId, allPumps, drivers }: any) {
  ...
  const pendDispatch = orders.filter((o: Order) => o.status === 'ready').length;
  const pendReturns = allPumps.filter((p: Pump) => p.status === 'with_driver' && p.pharmacyId === pharmacyId).length;
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col relative text-slate-900">
        <header className="p-8 pt-12 bg-white border-b border-slate-100 flex justify-between items-end shadow-sm">
          <div><h2 className="text-2xl font-black italic uppercase tracking-tighter">{user.city || 'Office'}</h2><p className="text-[10px] font-black text-emerald-600 mt-1 uppercase tracking-widest">{user.role.replace('_', ' ')}</p></div>
          <div className="flex gap-2">
            <button onClick={() => setSection('menu')} className="p-4 bg-slate-100 rounded-2xl"><ChevronLeft/></button>
            <button onClick={onLogout} className="p-4 bg-red-50 text-red-500 rounded-2xl shadow-sm"><LogOut/></button>
          </div>
        </header>
        <div className="flex-1 overflow-y-auto p-6 pb-24 text-slate-900">
            {section==='menu' && (
              <div className="grid grid-cols-2 gap-4 animate-in fade-in">
                ...
                <MenuCard icon={<Truck/>} label="Dispatch" color="bg-indigo-500" onClick={()=>setSection('create_delivery')} badge={pendDispatch}/>
                <MenuCard icon={<RotateCcw/>} label="RECEIVE RETURNS" color="bg-blue-600" onClick={()=>setSection('receive_returns')} full badge={pendReturns}/>
...
function PharmacyStaffView({ orders, pumps, clients, onLogout, user, pharmacyId, allPumps, drivers }: any) {
  const [section, setSection] = useState<'menu'|'add_pump'|'list_pumps'|'add_client'|'list_clients'|'create_delivery'|'history'|'receive_returns'>('menu');
  const pendDispatch = orders.filter((o: Order) => o.status === 'ready').length;
  const pendReturns = allPumps.filter((p: Pump) => p.status === 'with_driver' && p.pharmacyId === pharmacyId).length;
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col relative text-slate-900">
        <header className="p-8 pt-12 bg-white border-b border-slate-100 flex justify-between items-end shadow-sm">
          <div><h2 className="text-2xl font-black italic uppercase">{user.name}</h2><p className="text-[10px] font-black text-emerald-600 mt-1 uppercase">BRANCH STAFF</p></div>
          {section!=='menu'?<button onClick={()=>setSection('menu')} className="p-4 bg-slate-100 rounded-2xl"><ChevronLeft/></button>:<button onClick={onLogout} className="p-4 bg-red-50 text-red-500 rounded-2xl"><LogOut/></button>}
        </header>
        <div className="flex-1 overflow-y-auto p-6 pb-24 text-slate-900">
            {section==='menu' && (
              <div className="grid grid-cols-2 gap-4 animate-in fade-in">
                <MenuCard icon={<Truck/>} label="Dispatch" color="bg-indigo-500" onClick={()=>setSection('create_delivery')} badge={pendDispatch} full />
                <MenuCard icon={<Plus/>} label="New Asset" color="bg-indigo-600" onClick={()=>setSection('add_pump')}/>
                <MenuCard icon={<Database/>} label="Inventory" color="bg-[#0f172a]" onClick={()=>setSection('list_pumps')}/>
                <MenuCard icon={<UserPlus/>} label="New Patient" color="bg-emerald-600" onClick={()=>setSection('add_client')}/>
                <MenuCard icon={<Users/>} label="Patients" color="bg-slate-800" onClick={()=>setSection('list_clients')}/>
            ...
            {section==='add_pump' && <AddAssetForm onFinish={()=>setSection('menu')} pharmacyId={pharmacyId}/>}
            {section==='list_pumps' && <ListAssets pumps={pumps} readOnly />}
            {section==='add_client' && <AddClientForm onFinish={()=>setSection('menu')} pharmacyId={pharmacyId} user={user}/>}
            {section==='list_clients' && <ListClients clients={clients} readOnly />}
            {section==='create_delivery' && <CreateDispatchForm clients={clients} pumps={pumps} onFinish={()=>setSection('menu')} user={user} pharmacyId={pharmacyId}/>}
            {section==='history' && <HistoryLogView orders={orders} onBack={()=>setSection('menu')} />}
            {section==='receive_returns' && <ReturnProcessForm drivers={drivers} pumps={allPumps} onFinish={()=>setSection('menu')} pharmacyId={pharmacyId}/>}
        </div>
...
function AddAssetForm({ onFinish, pharmacyId }: any) {
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-right-4 text-slate-900">
      <div className="flex justify-between items-center"><h3 className="font-black text-xl italic uppercase">New Equipment</h3><button onClick={onFinish} className="bg-slate-100 p-2 rounded-xl text-xs font-bold">Back</button></div>
      ...
      <div className="space-y-4 bg-white p-6 rounded-4xl border border-slate-100 shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-2"><input className="w-full bg-slate-50 p-4 rounded-2xl outline-none font-bold text-sm text-slate-900" placeholder="Brand Name" value={form.brand} onChange={e=>setForm({...form, brand: e.target.value})} /><input className="w-full bg-slate-50 p-4 rounded-2xl outline-none font-bold text-sm text-slate-900" placeholder="Model" value={form.model} onChange={e=>setForm({...form, model: e.target.value})} /></div>
      </div>
      <button onClick={save} className="w-full bg-[#0f172a] text-white py-5 rounded-4xl font-black uppercase text-xs shadow-xl active:scale-95 transition-all text-white font-bold">Register Asset</button>
    </div>
  );
}
...
function AddClientForm({ onFinish, pharmacyId, user }: any) {
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-right-4 text-slate-900">
      <div className="flex justify-between items-center"><h3 className="font-black text-xl italic uppercase tracking-tighter">New Patient Entry</h3><button onClick={onFinish} className="bg-slate-100 p-2 rounded-xl text-xs font-bold transition-colors text-slate-500">Back</button></div>
      {msg && <div className="bg-emerald-50 text-emerald-600 p-3 rounded-xl text-center font-bold text-xs">{msg}</div>}
      <div className="bg-white p-6 rounded-4xl border border-slate-100 space-y-4 shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-2 text-slate-900"><input className="w-full bg-slate-50 p-4 rounded-2xl font-bold outline-none" placeholder="Street Address" value={form.address} onChange={e=>setForm({...form, address: e.target.value})} /><input className="w-full bg-slate-50 p-4 rounded-2xl font-bold outline-none" placeholder="City" value={form.city} onChange={e=>setForm({...form, city: e.target.value})} /></div>
      </div>
      <button onClick={save} className="w-full bg-emerald-600 text-slate-900 py-5 rounded-4xl font-black uppercase text-xs shadow-xl active:scale-95 transition-all text-white font-bold">Register Profile</button>
    </div>
  );
}
    ...
    <div className="space-y-4 pb-20 animate-in fade-in text-slate-900">
        {clients.map((c: Client) => (
          <div key={c.id} className="bg-white p-6 rounded-3xl border flex flex-col gap-1 shadow-sm relative border-slate-100">
            ...
            <h4 className="font-black text-slate-800 text-lg text-slate-800">{c.name}</h4><p className="text-xs text-slate-400 flex items-center gap-1"><Mail className="w-3 h-3" /> {c.email}</p><p className="text-xs text-slate-400 flex items-center gap-1"><MapPin className="w-3 h-3" /> {c.address}, {c.city}</p>
          </div>
        ))}
    </div>
  );
  ...
  return (
    <div className="space-y-6 animate-in zoom-in-95 text-slate-900">
      <div className="bg-white p-8 rounded-4xl shadow-xl border border-slate-100 space-y-6 text-slate-900"><SearchableSelect label="1. Select Patient" options={clients.map((c:Client)=>({label:c.name, value:c.id}))} value={cId} onChange={setCId} />
        ...
        {sP.length > 0 && (<div className="space-y-2 text-slate-900">{sP.map((p: Pump) => (<div key={p.id} className="flex justify-between items-center p-3 bg-indigo-50 rounded-xl border border-indigo-100 text-slate-900"><div><p className="font-black text-xs uppercase opacity-70">S/N: {p.code}</p></div><button onClick={()=>setSP(sP.filter((x: Pump)=>x.id!==p.id))} className="text-red-500 hover:bg-red-50 p-1 rounded-full"><XCircle className="w-5 h-5 text-red-500" /></button></div>))}</div>)}
      </div>
      <button onClick={create} disabled={!cId || sP.length === 0} className="w-full bg-[#0f172a] text-white py-6 rounded-4xl font-black uppercase text-xs shadow-xl active:scale-95 transition-all text-white font-bold">Initialize Dispatch</button>
    </div>
  );
}

function ReturnProcessForm({ drivers, pumps, onFinish, pharmacyId }: any) {
    const [driverId, setDriverId] = useState(""); const [selectedPumps, setSelectedPumps] = useState<string[]>([]); const [s1, setS1] = useState(""); const [s2, setS2] = useState(""); const [loading, setLoading] = useState(false);
    ...
    const processReturn = async () => {
        if (!s1 || !s2 || selectedPumps.length === 0) return; setLoading(true);
        const b = writeBatch(db);
        selectedPumps.forEach(id => b.update(doc(db, 'pumps', id), { status: 'available', currentDriverId: null, lastReview: `Returned on ${new Date().toLocaleDateString()}` }));
        await b.commit(); setLoading(false); onFinish();
    };
    return (
        <div className="space-y-6 p-4 animate-in zoom-in-95 text-slate-900">
            <div className="flex justify-between items-center text-slate-900"><h3 className="font-black text-xl italic uppercase text-slate-900">Inventory Reception</h3><button onClick={onFinish} className="bg-slate-100 p-2 rounded-xl text-xs font-bold text-slate-500">Back</button></div>
            <div className="bg-white p-6 rounded-4xl shadow-lg border border-slate-100 space-y-6 text-slate-900">
                <SearchableSelect label="1. Select Driver" options={indebtedDrivers.map((d:Employee)=>({label:d.name, value:d.id}))} value={driverId} onChange={(val: string) => { setDriverId(val); setSelectedPumps([]); }} />
                {driverId && driverPumps.length > 0 && (
                  <div className="space-y-2 text-slate-900">
                    <p className="text-[10px] font-black text-slate-400 uppercase ml-2">Equipment in driver possession</p>
                    {driverPumps.map((p: Pump) => (
                      <div key={p.id} onClick={() => setSelectedPumps((prev: string[]) => prev.includes(p.id) ? prev.filter((i: string)=>i!==p.id) : [...prev, p.id])} className={`flex justify-between items-center p-4 rounded-xl border-2 transition-all cursor-pointer ${selectedPumps.includes(p.id) ? 'border-emerald-500 bg-emerald-50' : 'border-slate-100'} text-slate-900`}>
                ...
                {selectedPumps.length > 0 && (<div className="space-y-4 pt-4 border-t border-slate-100 text-slate-900"><SignaturePad onSave={setS1} label="Driver Handover Signature" /><SignaturePad onSave={setS2} label="Staff Receipt Signature" /></div>)}
            </div>
            <button onClick={processReturn} disabled={loading || !s1 || !s2 || selectedPumps.length === 0} className="w-full bg-[#10b981] text-slate-900 py-5 rounded-4xl font-black uppercase text-xs active:scale-95 disabled:opacity-50 transition-all text-white shadow-emerald-500/20 font-bold">Confirm Handover</button>
        </div>
    );
}
...
function DriverApp({ orders, allPumps, user, onLogout, myCustodyPumps }: any) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col text-slate-900">
        <header className="p-8 pt-12 bg-white border-b border-slate-100 flex justify-between items-end shadow-sm"><div><h2 className="text-2xl font-black italic text-[#0f172a] uppercase tracking-tighter">{user.city || 'Fleet'}</h2><p className="text-[10px] font-black text-indigo-600 mt-1 uppercase tracking-widest">{user.name}</p></div><button onClick={onLogout} className="p-4 bg-slate-100 rounded-2xl transition-colors active:bg-slate-200 text-slate-950"><LogOut/></button></header>
        {myCustodyPumps.length > 0 && (<div className="mx-6 mt-4 bg-red-500 p-5 rounded-3xl text-white shadow-lg animate-pulse text-white"><div className="flex items-center gap-2 mb-2"><AlertTriangle/><span className="font-black text-xs uppercase tracking-widest">RETURN REQUIRED</span></div><div className="flex flex-wrap gap-2 text-white">{myCustodyPumps.map((p: Pump) => <span key={p.id} className="text-[9px] bg-white text-red-600 px-2 py-1 rounded font-black font-bold">{p.code}</span>)}</div></div>)}
        <div className="p-6 space-y-4 flex-1 overflow-y-auto pb-20 text-slate-900"> {orders.filter((o: Order)=>o.status==='ready').length === 0 ? <div className="text-center py-20 opacity-30 font-black text-xs uppercase tracking-widest">No active routes available</div> : orders.filter((o: Order)=>o.status==='ready').map((o: Order) => (<div key={o.id} className="bg-white p-8 rounded-4xl shadow-xl border border-slate-100 animate-in slide-in-from-bottom-8 duration-500 text-slate-900"><div className="flex justify-between mb-6 text-slate-900"><span className="text-[10px] font-black text-indigo-600 tracking-widest uppercase">#{o.orderCode}</span><span className="bg-slate-100 text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-tighter">READY</span></div><h3 className="text-2xl font-black text-slate-800 mb-2 leading-none text-slate-900">{o.clientName}</h3><p className="text-xs text-slate-500 mb-8 flex items-center gap-2 font-bold uppercase text-slate-400 text-slate-400 text-slate-400"><MapPin className="w-4 h-4 text-emerald-500"/> {o.address}</p><button onClick={()=>claim(o.id)} className="w-full bg-[#0f172a] text-white py-5 rounded-2xl font-black uppercase text-xs active:scale-95 transition-all shadow-xl text-white font-bold">Accept Dispatch</button></div>))}</div>
    </div>
  );
}
...
function DriverWorkflow({ order, allPumps, user }: any) {
    ...
    return (
      <div className="flex-1 flex flex-col bg-white text-slate-900 text-slate-900">
        ...
        <div className="flex-1 p-8 space-y-8 overflow-y-auto text-slate-900 text-slate-900">
          {step === 'pickup' ? (
            <div className="space-y-6 text-slate-900 text-slate-900">
              {pR.length > 0 && (<div className="bg-amber-100 p-6 rounded-3xl border-2 border-amber-300 animate-pulse text-amber-700 font-bold"><p className="text-xs font-black mb-3 uppercase flex items-center gap-2 text-amber-700 text-amber-700 text-amber-700"><AlertTriangle/> Return Debt to Office</p><div className="flex flex-wrap gap-2 text-slate-900">{pR.map((p: any)=>(<div key={p.id} className="bg-white px-3 py-1 rounded-lg text-xs font-bold shadow-sm">S/N: {p.code}</div>))}</div></div>)}
            ...
            <div className="space-y-6 text-slate-900 text-slate-900 text-slate-900">
              ...
              {cD.length > 0 && (<div className="space-y-4">{cD.map((p: any)=>(<div key={p.id} className="p-5 rounded-3xl border-2 flex justify-between items-center bg-white shadow-sm border-slate-100 text-slate-900"><div><p className="font-black text-lg text-indigo-950 text-indigo-950">#{p.code}</p><p className="text-[10px] text-red-500 font-black uppercase tracking-tighter text-red-500">Collect from patient</p></div><button onClick={()=>setDS({...dS, [p.id]:'collected'})} className={`p-4 rounded-2xl transition-all ${dS[p.id]==='collected'?'bg-emerald-500 text-white scale-110 shadow-lg shadow-emerald-500/30 text-white':'bg-slate-100 text-slate-400'}`}><CheckCircle/></button></div>))}</div>)}
              <div className="space-y-4 pt-4 text-slate-900 text-slate-900"><SignaturePad onSave={setS1} label="Driver Proof of Delivery" /><SignaturePad onSave={setS2} label="Patient Receipt Signature" /></div>
            </div>
          )}
        </div>
        <div className="p-6 border-t bg-white text-slate-900 text-slate-900"><button onClick={action} disabled={!s1 || !s2} className="w-full bg-[#10b981] text-slate-900 py-6 rounded-4xl font-black uppercase text-sm shadow-xl active:scale-95 transition-all shadow-xl text-slate-950 font-bold">Confirm Protocol Step</button></div>
      </div>
    );
}
...
function SelectionView({ onLogin, onRegister }: any) {
  return (
    <div className="min-h-screen bg-[#0f172a] flex flex-col items-center justify-center p-8 text-white relative overflow-hidden text-center text-white text-white">
        ...
        <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.4em] mb-16 opacity-50 text-white text-white">Smart Medical Logistics Platform</p>
        <div className="w-full max-w-xs space-y-4 text-white text-white">
            <button onClick={onLogin} className="w-full bg-white text-[#0f172a] py-6 rounded-4xl font-black uppercase text-xs flex items-center justify-between px-10 shadow-xl transition-all active:scale-95 text-[#0f172a] font-bold text-slate-950"><span>Portal Access</span><ArrowRight className="w-5 h-5 text-emerald-600"/></button>
            <button onClick={onRegister} className="w-full bg-white/5 border border-white/10 py-6 rounded-4xl font-black uppercase text-xs flex items-center justify-between px-10 hover:bg-white/10 transition-all active:scale-95 text-white font-bold text-white"><span>Create Account</span><Plus className="w-5 h-5 text-emerald-400"/></button>
        </div>
        <div className="absolute bottom-10 text-white">
            <button onClick={onLogin} className="text-[10px] font-black text-slate-600 uppercase tracking-widest hover:text-white transition-colors text-slate-500">Administrator Portal</button>
        </div>
    </div>
  );
}
...
function RegisterView({ onBack }: any) {
    ...
    const [authPharm, setAuthPharm] = useState<any>(null);
    ...
    useEffect(() => {
        if (form.role === 'pharmacy_staff' && form.adminPin.length === 4) {
            const search = async () => {
                ...
                if (!s.empty) setAuthPharm({ id: s.docs[0].id, name: s.docs[0].data().name });
                else setAuthPharm(null);
            };
            search();
        } else { setAuthPharm(null); }
    ...
    const register = async () => {
        ...
        try {
            ...
            if (form.role === 'pharmacy_staff') {
                if (!authPharm) { alert("PIN Authorization Failed."); setLoad(false); return; }
                pId = authPharm.id;
            }
    ...
    if (newPin) return (<div className="min-h-screen bg-[#0f172a] flex flex-col items-center justify-center p-8 text-white text-center shadow-inner text-white text-white text-white text-white text-white text-white text-white"><h2 className="text-3xl font-black italic mb-4 uppercase text-white tracking-tighter text-white">SUCCESS</h2><p className="text-sm text-slate-400 mb-2 uppercase tracking-widest text-white text-white text-white">Your Access PIN:</p><p className="text-7xl font-black text-emerald-400 mb-10 tracking-tighter text-emerald-400 text-emerald-400 text-emerald-400">{newPin}</p><button onClick={onBack} className="w-full bg-white text-[#0f172a] py-5 rounded-4xl font-black uppercase text-xs active:scale-95 text-slate-950 shadow-emerald-500/20 text-slate-950 font-bold text-white text-white">Login Now</button></div>);
    
    return (
        <div className="min-h-screen bg-[#0f172a] flex flex-col p-8 text-white overflow-y-auto pb-20 text-white text-white text-white">
            <button onClick={onBack} className="self-start p-4 bg-white/5 rounded-2xl mb-8 transition-colors hover:bg-white/10 text-white text-white text-white"><ChevronLeft className="text-white text-white text-white"/></button>
            ...
            <div className="flex flex-wrap gap-2 mb-8 text-white text-white text-white">
                <button onClick={() => setForm({...form, role: 'driver'})} className={`flex-1 py-4 rounded-2xl font-black text-[9px] uppercase border transition-all ${form.role === 'driver' ? 'bg-indigo-600 border-indigo-600 shadow-lg text-white' : 'bg-transparent border-white/10 text-slate-400 text-slate-400'}`}>Driver Registration</button>
                <button onClick={() => setForm({...form, role: 'pharmacy_admin'})} className={`flex-1 py-4 rounded-2xl font-black text-[9px] uppercase border transition-all ${form.role === 'pharmacy_admin' ? 'bg-indigo-600 border-indigo-600 shadow-lg text-white' : 'bg-transparent border-white/10 text-slate-400 text-slate-400'}`}>Pharmacy Registration</button>
            ...
            <div className="space-y-4 text-white text-white text-white">
                ...
                {form.role === 'pharmacy_staff' && (
                  <div className="space-y-3 p-4 bg-white/5 rounded-3xl border border-white/10 text-white text-white text-white text-white text-white">
                    <input className="w-full bg-white/5 p-4 rounded-2xl font-black border border-white/10 outline-none text-center text-lg text-white text-white text-white" placeholder="Enter Admin Secret PIN" maxLength={4} value={form.adminPin} onChange={e=>setForm({...form, adminPin: e.target.value})} />
                    {authPharm && (<div className="p-3 bg-emerald-500/20 border border-emerald-500/50 rounded-xl animate-in zoom-in-95 text-center text-white text-white text-white text-white"><p className="text-[10px] uppercase font-black text-emerald-400 text-emerald-400 text-emerald-400 text-emerald-400">Authorized by:</p><p className="font-bold text-white uppercase text-white text-white text-white">{authPharm.name}</p></div>)}
                  </div>
                )}
                
                {/* FIX: VISIBILIDAD DE SELECTORES - LETRA NEGRA FONDO BLANCO */}
                <div className="space-y-1 text-white text-white text-white">
                  ...
                  <select className="w-full bg-white p-4 rounded-2xl border border-slate-200 text-sm text-slate-900 font-bold outline-none text-slate-900" value={form.country} onChange={e=>setForm({...form, country: e.target.value, state: ''})}>
                    {Object.keys(LOCATIONS).map(k=><option key={k} value={k} className="text-slate-900">{LOCATIONS[k].label}</option>)}
                  </select>
                </div>
                
                <div className="grid grid-cols-2 gap-2 text-slate-900 text-slate-900 text-slate-900">
                  <div className="space-y-1 text-white text-white text-white">
                    <label className="text-[10px] font-black text-indigo-400 ml-3 uppercase text-indigo-400 text-indigo-400">State</label>
                    <select className="w-full bg-white p-4 rounded-2xl border border-slate-200 text-sm text-slate-900 font-bold outline-none text-slate-900" value={form.state} onChange={e=>setForm({...form, state: e.target.value})}>
                      <option value="" className="text-slate-900">Select...</option>
                      {LOCATIONS[form.country]?.states.map((s: string)=><option key={s} value={s} className="text-slate-900">{s}</option>)}
                    </select>
                  </div>
                  <div className="space-y-1 text-white text-white text-white">
                    <label className="text-[10px] font-black text-indigo-400 ml-3 uppercase text-indigo-400 text-indigo-400">City</label>
                    <input className="bg-white p-4 h-[54px] w-full rounded-2xl border border-slate-200 font-bold outline-none text-slate-900 placeholder-slate-400 text-sm text-slate-900 text-slate-900" placeholder="City" value={form.city} onChange={e=>setForm({...form, city: e.target.value})} />
                  </div>
                </div>

                <input className="w-full bg-white/5 p-4 rounded-2xl font-bold border border-white/10 outline-none text-white placeholder-white/30 text-white text-white text-white" placeholder="Full Legal Name" value={form.name} onChange={e=>setForm({...form, name: e.target.value})} /><input className="w-full bg-white/5 p-4 rounded-2xl font-bold border border-white/10 outline-none text-white placeholder-white/30 text-white text-white text-white text-white text-white" placeholder="Your Email" value={form.email} onChange={e=>setForm({...form, email: e.target.value})} />
                
                {form.role !== 'pharmacy_admin' && (
                    <SignaturePad onSave={setSign} label="Personal Identity Signature (Mandatory)" />
                )}
                
                <button onClick={register} disabled={load || (form.role === 'pharmacy_staff' && !authPharm) || (form.role !== 'pharmacy_admin' && !sign)} className="w-full bg-emerald-600 py-6 rounded-4xl font-black uppercase text-sm mt-4 shadow-xl active:scale-95 disabled:opacity-50 text-slate-950 text-slate-950 text-slate-950 text-slate-950 text-slate-950 font-bold text-white text-white"> {load ? "Saving..." : "Create Account"}</button>
            </div>
        </div>
    );
}
...
function LoginView({ pin, error, onInput, onClear, onBack }: any) {
  return (
    <div className="min-h-screen bg-[#0f172a] flex flex-col items-center p-8 text-white text-white text-white">
        <button onClick={onBack} className="self-start p-4 bg-white/5 rounded-2xl mb-8 transition-colors hover:bg-white/10 text-white text-white text-white"><ChevronLeft className="text-white text-white text-white"/></button>
        <h2 className="text-3xl font-black italic mb-2 tracking-tighter uppercase text-center text-white text-white text-white">Secure Access</h2>
        <div className="flex gap-4 mb-16 h-8 mt-10 text-white text-white text-white text-white">{[...Array(4)].map((_, i) => (<div key={i} className={`w-3 h-3 rounded-full transition-all duration-300 ${pin.length > i ? 'bg-emerald-500 scale-150 shadow-lg shadow-emerald-500/50' : 'bg-slate-800'}`} />))}</div>
...
export default function App() {
  ...
  useEffect(() => {
    if (!user) return;
    const sub = (col: string, setter: (data: any[]) => void) => onSnapshot(collection(db, col), (s: QuerySnapshot<DocumentData>) => setter(s.docs.map((d: QueryDocumentSnapshot<DocumentData>) => ({id: d.id, ...d.data()}))), (e: any) => console.error(e));
...
if (errorMsg)
  return (
    <div className="p-4 text-red-500">
      <p>{errorMsg}</p>
      <button onClick={manualRetry} className="underline">
        Retry
      </button>
    </div>
  );
  ...
  if (view === 'pharmacy_staff' && activeEmployee) {
    ...
    return <PharmacyStaffView user={activeEmployee} pharmacyId={pId} orders={orders.filter((o: Order) => o.pharmacyId === pId)} pumps={pumps.filter((p: Pump) => p.pharmacyId === pId)} clients={clients.filter((c: Client) => c.pharmacyId === pId)} staff={employees.filter((e: Employee) => e.pharmacyId === pId && e.role === 'pharmacy_staff')} drivers={drivers} onLogout={() => { setActiveEmployee(null); setView('selection'); setPin(""); }} allPumps={pumps} />;
  }
  ...
  if (view === 'driver_market' && activeEmployee) {
    ...
    return <DriverApp user={activeEmployee} orders={local} allOrders={orders} allPumps={pumps} myCustodyPumps={myCustody} onLogout={() => { setActiveEmployee(null); setView('selection'); setPin(""); }} />;
  }

...
function LoadingScreen() { 
  return (
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center text-white p-8">
      ...
      <p className="font-black uppercase text-[10px] tracking-widest opacity-40">Initializing Dispatcher Pro...</p>
    </div>
  ); 
}
...
function ErrorScreen({ msg, onRetry }: { msg: string, onRetry: () => void }) {
  return (
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center text-white p-8 text-center">
        ...
        <h2 className="text-xl font-bold mb-2 uppercase italic tracking-tighter">System Connection Error</h2>
        <p className="text-slate-400 text-xs mb-8 max-w-xs mx-auto">{msg}</p>
  ...
  return (
    <div className="bg-white p-3 rounded-2xl border-2 border-slate-200 shadow-sm">
        ...
        <div className="border-2 border-dashed border-slate-300 rounded-xl bg-slate-50 overflow-hidden touch-none relative h-32">
            ...
            {!hasSignature && <div className="absolute inset-0 flex items-center justify-center pointer-events-none text-slate-300 text-[10px] font-black uppercase">Sign with finger</div>}
        </div>
    </div>
  );
}
...
function SearchableSelect({ label, options, value, onChange, placeholder }: any) {
  ...
  useEffect(() => { if (!value) setQueryTerm(""); }, [value]);
  const filtered = options.filter((o:any) => (o.label || "").toLowerCase().includes(queryTerm.toLowerCase()));
  return (
        ...
        {isOpen && (
          <><div className="fixed inset-0 z-10" onClick={() => setIsOpen(false)}></div><div className="absolute top-full left-0 w-full bg-white mt-2 rounded-2xl shadow-2xl border border-slate-100 max-h-60 overflow-y-auto z-20 text-slate-900 text-left">{filtered.length === 0 ? (
                ...
                filtered.map((opt:any) => (
                  <div key={opt.value} className="p-4 border-b border-slate-50 hover:bg-indigo-50 cursor-pointer transition-colors text-left" onClick={() => { onChange(opt.value); setQueryTerm(""); setIsOpen(false); }}>
...
function ManualView({ onBack }: { onBack: () => void }) {
  const steps = [
    { title: "Account Activation", content: "To join as a business partner, obtain an Activation Key. Register using 'Pharmacy Registration' and link your location to manage local drivers.", icon: <Key className="w-5 h-5"/> },
    { title: "Inventory", content: "Load equipment using Serial Numbers (S/N). Track status: Available, With Client, or With Driver.", icon: <Database className="w-5 h-5"/> },
    { title: "Smart Dispatch", content: "The system alerts you if a patient has unreturned equipment before you send a new order.", icon: <Send className="w-5 h-5"/> },
    { title: "Return Protocol", content: "Drivers must return assets to the office. Both parties must sign to clear the driver's custody debt.", icon: <ShieldCheck className="w-5 h-5"/> }
  ];
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col text-slate-900 animate-in fade-in">
        <header className="p-6 bg-[#0f172a] text-white flex justify-between items-center shadow-lg border-b border-indigo-500/20">
            <div className="flex items-center gap-3">
                ...
                <div><h2 className="text-xl font-black italic uppercase tracking-tighter">User Manual</h2><p className="text-[8px] font-bold text-indigo-400 uppercase tracking-widest">Protocol v2.4</p></div>
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 pb-32">
            <div className="bg-indigo-600 p-8 rounded-4xl text-white shadow-2xl shadow-indigo-200 relative overflow-hidden">
                <div className="relative z-10"><h3 className="text-2xl font-black italic mb-2 uppercase">Core Logistics Guide</h3><p className="text-xs opacity-90 leading-relaxed">Follow these steps to maintain a 100% accurate chain of custody for all medical equipment.</p></div>
                <Info className="absolute -right-4 -bottom-4 w-32 h-32 opacity-10 rotate-12" />
            </div>
            {steps.map((s, i) => (
                <div key={i} className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
                    <div className="flex items-center gap-3 mb-3 text-indigo-600 font-black uppercase text-sm">{s.icon} {s.title}</div>
        ...
        <div className="p-6 fixed bottom-0 left-0 w-full bg-slate-50/80 backdrop-blur-md">
            <button onClick={onBack} className="w-full bg-[#0f172a] text-white py-5 rounded-3xl font-black uppercase text-xs active:scale-95">Go Back</button>
        </div>
    </div>
  );
}
...
function HistoryLogView({ orders, onBack }: any) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col text-slate-900">
        <div className="p-6 flex justify-between items-center bg-white border-b sticky top-0 z-20"><h2 className="text-xl font-black italic uppercase">History Log</h2><button onClick={onBack} className="bg-slate-100 p-2 rounded-xl text-xs font-bold text-slate-500">Back</button></div>
...
/* ==========================================================================
   6. LOGISTICS FORMS & ADMIN MODULES
   ========================================================================== */
...
function AddPumpForm({ onFinish, pharmacyId }: any) {
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-right-4 text-slate-900">
      <div className="flex justify-between items-center"><h3 className="font-black text-xl italic uppercase text-slate-900">Register Asset</h3><button onClick={onFinish} className="bg-slate-100 p-2 rounded-xl text-xs font-bold text-slate-500">Back</button></div>
      ...
      <button onClick={save} className="w-full bg-[#0f172a] text-white py-5 rounded-4xl font-black uppercase text-xs shadow-xl active:scale-95 transition-all text-white shadow-indigo-500/20">Confirm Asset Registration</button>
    </div>
  );
}
  ...
  return (
    <div className="space-y-6 animate-in zoom-in-95 text-slate-900">
      <div className="bg-white p-8 rounded-4xl shadow-xl border border-slate-100 space-y-6 text-slate-900 text-slate-900">
        ...
        {debts.length > 0 && (
          <div className="bg-red-50 p-4 rounded-2xl border border-red-100 animate-pulse shadow-sm">
            <p className="text-[10px] font-black text-red-600 mb-2 uppercase flex items-center gap-1 text-red-600"><AlertTriangle className="w-3 h-3"/> UNRETURNED ASSETS AT HOME:</p>
            <div className="flex flex-wrap gap-1">{debts.map((p: Pump) => <span key={p.id} className="text-[10px] bg-white px-2 py-1 rounded font-mono font-bold text-red-700 border border-red-200">{p.code}</span>)}</div>
          </div>
        )}
          ...
          <div className="space-y-2 text-slate-900">
            {sP.map((p: Pump) => (
              <div key={p.id} className="flex justify-between items-center p-3 bg-indigo-50 rounded-xl border border-indigo-100">
                ...
                <button onClick={()=>setSP(sP.filter((x: Pump)=>x.id!==p.id))} className="text-red-500 hover:bg-red-50 p-1 rounded-full"><XCircle className="w-5 h-5"/></button>
              </div>
            ))}
          </div>
        )}
      </div>
      <button onClick={create} disabled={!cId || sP.length === 0} className="w-full bg-[#0f172a] text-white py-6 rounded-4xl font-black uppercase text-xs shadow-xl active:scale-95 transition-all text-white shadow-slate-900/10">Initialize Dispatch</button>
    </div>
  );
}
...
function ReturnPumpsForm({ drivers, pumps, onFinish, pharmacyId }: any) {
    ...
    return (
        <div className="space-y-6 p-4 animate-in zoom-in-95 text-slate-900">
            <div className="flex justify-between items-center text-slate-900 text-slate-900"><h3 className="font-black text-xl italic uppercase text-slate-900">Inventory Reception</h3><button onClick={onFinish} className="bg-slate-100 p-2 rounded-xl text-xs font-bold text-slate-500">Back</button></div>
            <div className="bg-white p-6 rounded-4xl shadow-lg border border-slate-100 space-y-6 text-slate-900">
                ...
                {driverId && driverPumps.length > 0 && (
                  <div className="space-y-2 text-slate-900 text-slate-900">
                    <p className="text-[10px] font-black text-slate-400 uppercase ml-2 text-slate-400 text-slate-400">Equipment in driver possession</p>
            ...
            <button onClick={processReturn} disabled={loading || !signDriver || !signStaff || selectedPumps.length === 0} className="w-full bg-[#10b981] text-slate-900 py-5 rounded-4xl font-black uppercase text-xs active:scale-95 disabled:opacity-50 shadow-xl transition-all text-white shadow-emerald-500/20">Close Chain of Custody</button>
        </div>
    );
}
...
function StaffManager({ staff, pharmacyId }: any) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in text-slate-900 text-slate-900"> 
      {p ? (
        <div className="bg-emerald-500 text-white p-8 rounded-4xl text-center shadow-lg text-white">
          ...
          <button onClick={()=>setP("")} className="mt-6 text-xs underline text-white">Register another member</button>
        </div>
      ) : (
...
function SuperAdminView({ onLogout }: any) {
    ...
    return (<div className="min-h-screen bg-[#0f172a] text-white p-8 max-w-md mx-auto flex flex-col text-white"><header className="flex justify-between items-end mb-10"><div><h2 className="text-3xl font-black italic tracking-tighter uppercase">Master Hub</h2><p className="text-[10px] text-emerald-500 font-bold uppercase tracking-widest opacity-60 text-emerald-500">Global Licenses</p></div><button onClick={onLogout} className="p-4 bg-white/10 rounded-2xl transition-colors hover:bg-white/20 text-white"><LogOut/></button></header><div className="space-y-6 flex-1 overflow-y-auto"><div className="bg-white/5 p-8 rounded-4xl border border-white/10 space-y-4 shadow-2xl text-white"><h3 className="font-bold flex gap-2 text-white text-sm uppercase tracking-widest text-white text-white"><Ticket className="text-emerald-500"/> Generate Activation Key</h3><input className="w-full bg-slate-800 p-4 rounded-2xl outline-none text-sm border border-white/5 font-bold text-white uppercase placeholder-white/20" placeholder="Partner email" value={e} onChange={v=>setE(v.target.value)}/><button onClick={gen} className="w-full bg-emerald-600 text-slate-950 py-5 rounded-4xl font-black uppercase text-xs active:scale-95 shadow-lg shadow-emerald-500/20 text-slate-950">Create Key</button></div><div className="space-y-3">{l.map((x: any)=>(<div key={x.id} className="bg-white/5 p-5 rounded-3xl flex justify-between border border-white/5 items-center"><div><p className="font-black text-emerald-400 tracking-wider text-lg text-emerald-400">{x.code}</p><p className="text-[10px] opacity-40 uppercase tracking-widest">{x.assignedEmail}</p></div><span className={`text-[8px] px-3 py-1 rounded-full uppercase font-black ${x.status === 'active' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'}`}>{x.status}</span></div>))}</div></div></div>);
}
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col relative text-slate-900">
        <header className="p-8 pt-12 bg-white border-b border-slate-100 flex justify-between items-end shadow-sm text-slate-900 text-slate-900">
          <div><h2 className="text-2xl font-black italic text-slate-900 uppercase tracking-tighter text-slate-900">{user.city} Office</h2><p className="text-[10px] font-black text-emerald-600 mt-1 uppercase tracking-widest text-emerald-600">{user.role.replace('_', ' ')}</p></div>
          <div className="flex gap-2">
            <button onClick={onOpenManual} className="p-4 bg-indigo-50 text-indigo-600 rounded-2xl transition-colors hover:bg-indigo-100 text-indigo-600 text-indigo-600"><BookOpen className="w-6 h-6"/></button>
...
function DriverApp({ orders, allPumps, user, onLogout, myCustodyPumps, onOpenManual, allOrders }: any) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col text-slate-900 text-slate-900 text-slate-900">
        <header className="p-8 pt-12 bg-white border-b border-slate-100 flex justify-between items-end shadow-sm text-slate-900 text-slate-900"><div><h2 className="text-2xl font-black italic text-[#0f172a] uppercase tracking-tighter">{user.city} Fleet</h2><p className="text-[10px] font-black text-indigo-600 mt-1 uppercase tracking-widest text-indigo-600">{user.name}</p></div><div className="flex gap-2"><button onClick={onOpenManual} className="p-4 bg-indigo-50 text-indigo-600 rounded-2xl text-indigo-600"><BookOpen className="w-6 h-6 text-indigo-600"/></button><button onClick={onLogout} className="p-4 bg-slate-100 rounded-2xl text-slate-900 transition-colors active:bg-slate-200"><LogOut/></button></div></header>
        {myCustodyPumps.length > 0 && (<div className="mx-6 mt-4 bg-red-500 p-5 rounded-3xl text-white shadow-lg animate-pulse text-white"><div className="flex items-center gap-2 mb-2 text-white"><AlertTriangle/><span className="font-black text-xs uppercase tracking-widest text-white text-white">HANDOVER REQUIRED</span></div><div className="flex flex-wrap gap-2 text-white text-white">{myCustodyPumps.map((p: Pump) => <span key={p.id} className="text-[9px] bg-white text-red-600 px-2 py-1 rounded font-black text-red-600">{p.code}</span>)}</div></div>)}
...
function DriverWorkflow({ order, allPumps, user, pharmacyId }: any) {
    ...
    const action = async () => {
        ...
        if (step === 'pickup') {
            ...
            retIds.forEach(id => b.update(doc(db, 'pumps', id), { status: 'with_driver', currentClientId: null, currentDriverId: user.id }));
          ...
          {step === 'pickup' ? (
            <div className="space-y-6 text-slate-900 text-slate-900 text-slate-900">
              {pR.length > 0 && (
                <div className="bg-amber-100 p-6 rounded-3xl border-2 border-amber-300 animate-pulse text-amber-700 text-amber-700">
                  <p className="text-xs font-black text-amber-700 mb-3 uppercase flex items-center gap-2 text-amber-700"><AlertTriangle/> Return Debt to Office</p>
                  <div className="flex flex-wrap gap-2 text-amber-700">{pR.map((p: Pump)=>(<div key={p.id} className="bg-white px-3 py-1 rounded-lg text-xs font-bold shadow-sm text-slate-900">S/N: {p.code}</div>))}</div>
                </div>
              )}
            ...
            <div className="space-y-6 text-slate-900 text-slate-900">
              <div className="p-8 bg-[#0f172a] rounded-4xl text-white shadow-xl text-white text-white">
                ...
                <p className="text-sm opacity-60 flex items-center gap-2 font-bold text-white/50 text-white/50"><MapPin className="w-4 h-4 text-white"/> {order.address}</p>
              </div>
              {cD.length > 0 && (
                <div className="space-y-4">
                  {cD.map((p: Pump)=>(
                    <div key={p.id} className="p-5 rounded-3xl border-2 flex justify-between items-center bg-white shadow-sm border-slate-100 text-slate-900 text-slate-900">
                      ...
                      <button onClick={()=>setDS({...dS, [p.id]:'collected'})} className={`p-4 rounded-2xl transition-all ${dS[p.id]==='collected'?'bg-emerald-500 text-white scale-110 shadow-lg shadow-emerald-500/30 text-white text-white':'bg-slate-100 text-slate-400 hover:bg-slate-200'}`}><CheckCircle/></button>
                    </div>
                  ))}
                </div>
              )}
              <div className="space-y-4 pt-4 text-slate-900 text-slate-900"><SignaturePad onSave={setS1} label="Proof of Delivery Signature" /><SignaturePad onSave={setS2} label="Patient Receipt Signature" /></div>
            </div>
          )}
        </div>
        <div className="p-6 border-t bg-white text-slate-900 text-slate-900 text-slate-900"><button onClick={action} disabled={!s1 || (step==='delivery'&&!s2)} className="w-full bg-[#10b981] text-slate-900 py-6 rounded-4xl font-black uppercase text-sm shadow-xl active:scale-95 transition-all shadow-xl text-slate-950 text-slate-950">Confirm Protocol Step</button></div>
      </div>
    );
}

/* ==========================================================================
   8. MAIN LOGIC COMPONENTS
   ========================================================================== */
...
function SelectionView({ onLogin, onRegister, onOpenManual }: { onLogin: () => void, onRegister: () => void, onOpenManual: () => void }) {
  return (
    <div className="min-h-screen bg-[#0f172a] flex flex-col items-center justify-center p-8 text-white relative overflow-hidden text-center text-white text-white">
        <div className="w-24 h-24 bg-emerald-500 rounded-3xl flex items-center justify-center mb-8 shadow-2xl animate-bounce duration-[3000ms] shadow-emerald-500/20 text-white text-white text-white text-white"><Truck className="w-12 h-12 text-[#0f172a] text-[#0f172a]" /></div>
        <h1 className="text-4xl font-black italic tracking-tighter mb-1 uppercase text-white text-white">Dispatcher Pro</h1>
        <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.4em] mb-16 text-center text-white/50 opacity-50 text-white">Smart Medical Logistics Platform</p>
        <div className="w-full max-w-xs space-y-4 text-white text-white text-white">
            <button onClick={onLogin} className="w-full bg-white text-[#0f172a] py-6 rounded-4xl font-black uppercase text-xs flex items-center justify-between px-10 shadow-xl transition-all active:scale-95 text-[#0f172a] text-[#0f172a] text-[#0f172a]"><span>Portal Access</span><ArrowRight className="w-5 h-5 text-emerald-600 text-emerald-600"/></button>
            <button onClick={onRegister} className="w-full bg-white/5 border border-white/10 py-6 rounded-4xl font-black uppercase text-xs flex items-center justify-between px-10 hover:bg-white/10 transition-all active:scale-95 text-white text-white"><span>Register Business</span><Plus className="w-5 h-5 text-emerald-400 text-emerald-400"/></button>
            <button onClick={onOpenManual} className="w-full mt-4 flex items-center justify-center gap-2 text-[10px] font-black text-indigo-400 uppercase tracking-widest hover:text-indigo-300 transition-colors text-indigo-400 text-indigo-400 text-indigo-400"><BookOpen className="w-4 h-4"/> Help & Manual</button>
        </div>
        <div className="absolute bottom-10">
            <button onClick={onLogin} className="text-[10px] font-black text-slate-600 uppercase tracking-widest hover:text-white transition-colors text-slate-600 text-slate-600 text-slate-600">Internal Ops Only</button>
        </div>
    </div>
  );
}
...
function LoginView({ pin, error, onInput, onClear, onBack }: any) {
  return (
    <div className="min-h-screen bg-[#0f172a] flex flex-col items-center p-8 text-white text-white text-white text-white text-white text-white">
        <button onClick={onBack} className="self-start p-4 bg-white/5 rounded-2xl mb-8 transition-colors hover:bg-white/10 text-white text-white text-white text-white"><ChevronLeft className="text-white text-white text-white"/></button>
        <h2 className="text-3xl font-black italic mb-2 tracking-tighter uppercase text-white text-center text-white text-white">Secure Access</h2>
        <div className="flex gap-4 mb-16 h-8 mt-10 text-white text-white text-white">{[...Array(4)].map((_, i) => (<div key={i} className={`w-3 h-3 rounded-full transition-all duration-300 ${pin.length > i ? 'bg-emerald-500 scale-150 shadow-lg shadow-emerald-500/50' : 'bg-slate-800'}`} />))}</div>
        ...
        <div className="grid grid-cols-3 gap-4 w-full max-w-[300px] text-white text-white text-white">
            ...
            <button onClick={onClear} className="h-20 rounded-3xl bg-red-500/10 text-red-500 font-black text-xs uppercase flex items-center justify-center active:scale-95 text-red-500 text-red-500 text-red-500 text-red-500">CLR</button>
            <button onClick={() => onInput("0")} className="h-20 rounded-3xl bg-white/5 text-2xl font-black text-white text-white text-white text-white text-white">0</button>
        </div>
    </div>
  );
}
...
function RegisterView({ onBack }: any) {
    ...
    if (newPin) return (<div className="min-h-screen bg-[#0f172a] flex flex-col items-center justify-center p-8 text-white text-center shadow-inner text-white text-white text-white text-white text-white"><h2 className="text-3xl font-black italic mb-4 uppercase text-white tracking-tighter text-white text-white text-white text-white text-white text-white text-white">Registration Complete</h2><p className="text-sm text-slate-400 mb-2 uppercase tracking-widest text-white text-white text-white text-white text-white text-white text-white text-white text-white">Your Private Access PIN:</p><p className="text-7xl font-black text-emerald-400 mb-10 tracking-tighter text-emerald-400 text-emerald-400 text-emerald-400 text-emerald-400 text-emerald-400 text-emerald-400 text-emerald-400 text-emerald-400">{newPin}</p><button onClick={onBack} className="w-full bg-white text-[#0f172a] py-5 rounded-4xl font-black uppercase text-xs active:scale-95 transition-all text-slate-950 text-slate-950 text-slate-950 text-slate-950 text-slate-950">Login Now</button></div>);
    return (
        <div className="min-h-screen bg-[#0f172a] flex flex-col p-8 text-white overflow-y-auto pb-20 text-white text-white text-white text-white text-white text-white text-white">
            <button onClick={onBack} className="self-start p-4 bg-white/5 rounded-2xl mb-8 transition-colors hover:bg-white/10 text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white"><ChevronLeft className="text-white text-white text-white text-white text-white text-white text-white text-white text-white"/></button>
            <h2 className="text-3xl font-black italic mb-8 uppercase tracking-tighter text-white text-white text-white text-white">Account Setup</h2>
            <div className="grid grid-cols-2 gap-3 mb-8 text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white">
            ...
            <div className="space-y-4 text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white">
                ...
                <input className="w-full bg-white/5 p-4 rounded-2xl font-bold border border-white/10 outline-none text-white placeholder-white/30 text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white" placeholder="Full Legal Name" value={form.name} onChange={e=>setForm({...form, name: e.target.value})} />
                <input className="w-full bg-white/5 p-4 rounded-2xl font-bold border border-white/10 outline-none text-white placeholder-white/30 text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white" placeholder="Business Email" value={form.email} onChange={e=>setForm({...form, email: e.target.value})} />
                <div className="space-y-1">
                    <label className="text-[10px] font-black text-indigo-400 ml-3 uppercase text-indigo-400 text-indigo-400 text-indigo-400 text-indigo-400 text-indigo-400 text-indigo-400 text-indigo-400 text-indigo-400 text-indigo-400 text-indigo-400">Country</label>
                ...
                <div className="grid grid-cols-2 gap-2 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">
                    <div className="space-y-1 text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white">
                        ...
                        <select className="w-full bg-white/5 p-4 rounded-2xl border border-white/10 text-sm outline-none text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white" value={form.state} onChange={e=>setForm({...form, state: e.target.value})}><option value="" className="text-black text-black text-black text-black text-black text-black text-black text-black text-black text-black text-black text-black text-black">Select...</option>{LOCATIONS[form.country]?.states.map((s: string)=><option key={s} value={s} className="text-black text-black text-black text-black text-black text-black text-black text-black text-black text-black text-black text-black">{s}</option>)}</select>
                    </div>
                    <div className="space-y-1 text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white text-white">
                        <label className="text-[10px] font-black text-indigo-400 ml-3 uppercase text-indigo-400 text-indigo-400 text-indigo-400 text-indigo-400 text-indigo-400 text-indigo-400 text-indigo-400 text-indigo-400 text-indigo-400 text-indigo-400 text-indigo-400 text-indigo-400">City</label>
...
export default function App() {
  ...
  useEffect(() => {
    if (!user) return;
    const sub = (col: string, setter: any) => onSnapshot(collection(db, col), s => setter(s.docs.map(d => ({id: d.id, ...d.data()}))), (e: any) => console.error(e));
  ...
  const handlePinInput = (digit: string) => {
    if (pin.length < 4) {
      ...
      if (newPin.length === 4) {
        ...
        const dri = drivers.find(d => d.pin === newPin);

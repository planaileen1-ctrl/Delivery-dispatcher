...
/* ==========================================================================
   3. COMPONENTES DE UI BASE (CARGA PRIMERO)
   ========================================================================== */
...
function LoadingScreen() { 
  return (
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center text-white p-8">
      ...
      <p className="font-black uppercase text-[10px] tracking-[0.3em] opacity-30 text-center">Iniciando Dispatcher Pro...</p>
    </div>
  ); 
}
...
function ErrorScreen({ msg, onRetry }: { msg: string, onRetry: () => void }) {
  return (
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center text-white p-8 text-center">
        ...
        <h2 className="text-xl font-bold mb-2 uppercase italic tracking-tighter">Error del Sistema</h2>
        <p className="text-slate-400 text-xs mb-8 max-w-xs mx-auto">{msg}</p>
        <button onClick={onRetry} className="bg-emerald-500 text-slate-900 px-8 py-4 rounded-3xl font-black uppercase text-xs flex items-center gap-2 active:scale-95 transition-all mx-auto">
...
function SignaturePad({ onSave, label }: { onSave: (data: string) => void, label: string }) {
  ...
  const getCoordinates = (event: any, canvas: HTMLCanvasElement) => {
    ...
    // Soporte robusto para iPad/Móvil (Touch) y PC (Mouse)
    let clientX, clientY;
  ...
  return (
    <div className="bg-white p-3 rounded-2xl border-2 border-slate-200 shadow-sm">
        <div className="flex justify-between items-center mb-1">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1"><PenTool className="w-3 h-3"/> {label}</label>
...
/* ==========================================================================
   4. COMPONENTES DE NEGOCIO (LISTAS, HISTORIAL, FORMULARIOS)
   ========================================================================== */
...
function HistoryLogView({ orders, onBack }: any) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col text-slate-900">
        <div className="p-6 flex justify-between items-center bg-white border-b border-slate-100 sticky top-0 z-20"><h2 className="text-xl font-black italic text-slate-900 uppercase">Historial</h2><button onClick={onBack} className="bg-slate-100 p-2 rounded-xl text-xs font-bold text-slate-500">Volver</button></div>
        <div className="flex-1 overflow-y-auto p-4 space-y-4 pb-24">
            {hist.length === 0 ? <p className="text-center py-20 opacity-30 uppercase text-xs font-black">Historial vacío</p> : hist.map((o: any) => (
                <div key={o.id} className="bg-white p-5 rounded-3xl shadow-sm border border-slate-100 text-slate-900 animate-in">
                    <div className="flex justify-between items-start mb-2"><span className="text-[10px] font-black text-indigo-600 uppercase tracking-widest">#{o.orderCode}</span><span className={`text-[8px] px-2 py-1 rounded font-black uppercase ${o.status === 'delivered' ? 'bg-emerald-100 text-emerald-600' : 'bg-red-100 text-red-600'}`}>{o.status}</span></div>
                    <h4 className="font-bold text-slate-800">{o.clientName}</h4>
                    <p className="text-[10px] text-slate-400 mb-3 uppercase tracking-tighter">{o.address}</p>
    ...
    <div className="space-y-6 animate-in fade-in text-slate-900"> 
      {p ? (
        <div className="bg-emerald-500 text-white p-8 rounded-4xl text-center shadow-lg">
          <p className="font-bold mb-2 uppercase tracking-widest text-white text-white">PIN DEL EMPLEADO:</p>
          <p className="text-6xl font-black text-white text-white">{p}</p>
          <button onClick={()=>setP("")} className="mt-6 text-xs underline text-white text-white">Registrar otro personal</button>
        </div>
      ) : (
...
/* ==========================================================================
   5. PANELES Y DASHBOARDS (DEFINIDOS ANTES QUE APP)
   ========================================================================== */
    ...
    return (
        <div className="min-h-screen bg-[#0f172a] flex flex-col p-8 text-white overflow-y-auto pb-20">
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-2 text-slate-900">
                    <select className="bg-white/5 p-4 rounded-2xl border border-white/10 text-sm outline-none text-white text-white" onChange={e=>setForm({...form, state: e.target.value})}><option value="" className="text-black text-black">Estado...</option>{LOCATIONS['EC']?.states.map((s: any)=><option key={s} value={s} className="text-black text-black">{s}</option>)}</select>
...
function SuperAdminView({ onLogout, employees, drivers }: any) {
    ...
    return (<div className="min-h-screen bg-[#0f172a] text-white p-8 max-w-md mx-auto flex flex-col text-white text-white text-white"><header className="flex justify-between items-end mb-10 text-white text-white"><div><h2 className="text-3xl font-black italic tracking-tighter uppercase text-white">Master Hub</h2><p className="text-[10px] text-emerald-500 font-bold uppercase tracking-widest opacity-60 text-white text-white text-white text-white">Global Licenses</p></div><button onClick={onLogout} className="p-4 bg-white/10 rounded-2xl transition-colors hover:bg-white/20 text-white text-white"><LogOut/></button></header><div className="space-y-6 flex-1 overflow-y-auto text-white text-white text-white"><div className="bg-white/5 p-8 rounded-4xl border border-white/10 space-y-4 shadow-2xl text-white"><h3 className="font-bold flex gap-2 text-white text-white text-sm uppercase tracking-widest text-white"><Ticket className="text-emerald-500"/> Generate License</h3><input className="w-full bg-slate-800 p-4 rounded-2xl outline-none text-sm border border-white/5 font-bold text-white uppercase placeholder-white/20 text-white text-white text-white" placeholder="Email del cliente" value={e} onChange={v=>setE(v.target.value)}/><button onClick={gen} className="w-full bg-emerald-600 text-slate-900 py-5 rounded-4xl font-black uppercase text-xs active:scale-95 shadow-lg shadow-emerald-500/20 text-slate-900 text-slate-900 text-slate-900">Generar Código</button></div><div className="space-y-3">{l.map((x: any)=>(<div key={x.id} className="bg-white/5 p-5 rounded-3xl flex justify-between border border-white/5 items-center text-white text-white text-white text-white"><div><p className="font-black text-emerald-400 tracking-wider text-lg">{x.code}</p><p className="text-[10px] opacity-40 uppercase tracking-widest text-white text-white text-white text-white">{x.assignedEmail}</p></div><span className={`text-[8px] px-3 py-1 rounded-full uppercase font-black ${x.status === 'active' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'}`}>{x.status}</span></div>))}</div></div></div>);
}
          ...
          {step === 'pickup' ? (
            ...
            <div className="space-y-6">
              <div className="p-8 bg-[#0f172a] rounded-4xl text-white shadow-xl text-white">
                ...
                <p className="text-sm opacity-60 flex items-center gap-2 font-bold text-white/50 text-white/50 text-white/50 text-white/50"><MapPin className="w-4 h-4"/> {order.address}</p>
              </div>
              {cD.length > 0 && (
                <div className="space-y-4 text-slate-900 text-slate-900">
                  {cD.map((p: any)=>(
                    <div key={p.id} className="p-5 rounded-3xl border-2 flex justify-between items-center bg-white shadow-sm border-slate-100 text-slate-900 text-slate-900">
                      <div><p className="font-black text-lg text-indigo-950 text-indigo-950">#{p.code}</p><p className="text-[10px] text-red-500 font-black uppercase tracking-tighter text-red-500">Recoger del paciente</p></div>
              ...
              <div className="space-y-4 pt-4 text-slate-900 text-slate-900 text-slate-900 text-slate-900"><SignaturePad onSave={setS1} label="Firma Chofer (Entrega)" /><SignaturePad onSave={setS2} label="Firma Receptor / Paciente" /></div>
            </div>
          )}
        </div>
        <div className="p-6 border-t bg-white text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900"><button onClick={action} disabled={!s1 || (step==='delivery'&&!s2)} className="w-full bg-[#10b981] text-slate-900 py-6 rounded-4xl font-black uppercase text-sm shadow-xl active:scale-95 transition-all">Confirmar Paso</button></div>
      </div>
    );
}
...
function DriverApp({ orders, allPumps, user, onLogout, myCustodyPumps, allOrders }: any) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col text-slate-900 text-slate-900 text-slate-900">
        ...
        {myCustodyPumps.length > 0 && (<div className="mx-6 mt-4 bg-red-500 p-5 rounded-3xl text-white shadow-lg animate-pulse text-white text-white text-white text-white text-white text-white text-white"><div className="flex items-center gap-2 mb-2 text-white text-white text-white text-white text-white text-white text-white"><AlertTriangle/><span className="font-black text-xs uppercase tracking-widest text-white text-white text-white text-white text-white text-white text-white text-white">DEVOLUCIÓN REQUERIDA</span></div><div className="flex flex-wrap gap-2 text-white text-white text-white text-white text-white text-white text-white text-white">{myCustodyPumps.map((p:any) => <span key={p.id} className="text-[9px] bg-white text-red-600 px-2 py-1 rounded font-black text-red-600 text-red-600 text-red-600 text-red-600 text-red-600 text-red-600 text-red-600 text-red-600">{p.code}</span>)}</div></div>)}
        <div className="p-6 space-y-4 flex-1 overflow-y-auto pb-20 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900"> {orders.filter((o:any)=>o.status==='ready').length === 0 ? <div className="text-center py-20 opacity-30 font-black text-xs uppercase tracking-widest text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">Sin rutas disponibles</div> : orders.filter((o:any)=>o.status==='ready').map((o: any) => (<div key={o.id} className="bg-white p-8 rounded-4xl shadow-xl border border-slate-100 animate-in slide-in-from-bottom-8 duration-500 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900"><div className="flex justify-between mb-6 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900"><span className="text-[10px] font-black text-indigo-600 tracking-widest uppercase text-indigo-600 text-indigo-600 text-indigo-600 text-indigo-600 text-indigo-600 text-indigo-600 text-indigo-600 text-indigo-600 text-indigo-600 text-indigo-600">#{o.orderCode}</span><span className="bg-slate-100 text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-tighter text-slate-600 text-slate-600 text-slate-600 text-slate-600 text-slate-600 text-slate-600 text-slate-600 text-slate-600 text-slate-600 text-slate-600">{o.status}</span></div><h3 className="text-2xl font-black text-slate-800 mb-2 leading-none text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">{o.clientName}</h3><p className="text-xs text-slate-500 mb-8 flex items-center gap-2 font-bold uppercase text-slate-400 text-slate-400 text-slate-400 text-slate-400 text-slate-400 text-slate-400 text-slate-400 text-slate-400 text-slate-400 text-slate-400 text-slate-400 text-slate-400"><MapPin className="w-4 h-4 text-emerald-500"/> {o.address}</p><button onClick={()=>claimOrder(o.id)} className="w-full bg-[#0f172a] text-white py-5 rounded-2xl font-black uppercase text-xs active:scale-95 transition-all shadow-xl shadow-xl shadow-xl shadow-xl shadow-xl shadow-xl shadow-xl shadow-xl shadow-xl shadow-xl shadow-xl shadow-xl shadow-xl">Aceptar Ruta</button></div>))}</div>
    </div>
  );
}

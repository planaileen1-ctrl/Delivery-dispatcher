...
/* ==========================================================================
   3. COMPONENTES ATÓMICOS (DEFINIDOS PRIMERO)
   ========================================================================== */
...
function LoadingScreen() { 
  return (
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center text-white p-8">
      ...
      <p className="font-black uppercase text-[10px] tracking-[0.3em] opacity-30">Iniciando Dispatcher Pro...</p>
    </div>
  ); 
}
...
function ErrorScreen({ msg, onRetry }: { msg: string, onRetry: () => void }) {
  return (
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center text-white p-8 text-center">
        ...
        <h2 className="text-xl font-bold mb-2 uppercase italic tracking-tighter text-white">Error de Sistema</h2>
...
function MenuCard({ icon, label, color, onClick, full }: any) {
  return (
    <button onClick={onClick} className={`${color} ${full ? 'col-span-2' : ''} p-6 rounded-3xl text-white flex flex-col items-center justify-center gap-3 shadow-xl active:scale-[0.98] transition-all`}>
...
/* ==========================================================================
   4. COMPONENTES DE NEGOCIO (LISTAS, FORMULARIOS)
   ========================================================================== */

function ListPumps({ pumps, readOnly }: { pumps: Pump[], readOnly?: boolean }) {
  const remove = async(id:string) => { if(confirm("¿Eliminar?")) await deleteDoc(doc(db,'pumps',id)); };
  return (
...
function AddPumpForm({ onFinish, pharmacyId }: any) {
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-right-4 text-slate-900">
      ...
      <button onClick={save} className="w-full bg-indigo-600 text-white py-5 rounded-3xl font-black uppercase text-xs shadow-xl active:scale-95 transition-all">Guardar Bomba</button>
    </div>
  );
}
...
function AddClientForm({ onFinish, pharmacyId, user }: any) {
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-right-4 text-slate-900">
      ...
      {msg && <div className="bg-emerald-50 text-emerald-600 p-3 rounded-xl text-center font-bold text-xs">{msg}</div>}
      <div className="bg-white p-6 rounded-3xl border border-slate-100 space-y-4 shadow-sm text-slate-900">
      ...
      <button onClick={save} className="w-full bg-emerald-600 text-slate-900 py-5 rounded-3xl font-black uppercase text-xs shadow-xl active:scale-95 transition-all">Registrar Cliente</button>
    </div>
  );
}
...
function CreateDeliveryForm({ clients, pumps, onFinish, user, pharmacyId }: any) {
  ...
  return (
    <div className="space-y-6 animate-in zoom-in-95 text-slate-900">
      <div className="bg-white p-8 rounded-3xl shadow-xl border border-slate-100 space-y-6 text-slate-900">
      ...
      <button onClick={create} disabled={!cId || sP.length === 0} className="w-full bg-slate-900 text-white py-6 rounded-3xl font-black uppercase text-xs shadow-xl active:scale-95 transition-all disabled:opacity-50">Lanzar Despacho</button>
    </div>
  );
}
...
function StaffManager({ staff, pharmacyId }: any) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in text-slate-900"> 
      {p ? (
        <div className="bg-emerald-500 text-white p-8 rounded-3xl text-center shadow-lg">
        ...
        <div className="bg-white p-8 rounded-3xl shadow-xl border border-slate-100 space-y-4 text-slate-900">
          ...
          <button onClick={create} disabled={!s} className="w-full bg-slate-900 text-white py-5 rounded-2xl font-black uppercase text-xs disabled:opacity-50 active:scale-95 transition-all shadow-xl">Generar PIN Acceso</button>
        </div>
      )}
    </div>
  );
...
function HistoryLogView({ orders, allPumps, onBack }: any) {
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col text-slate-900">
            <div className="p-6 flex justify-between items-center bg-white border-b border-slate-100 sticky top-0 z-20"><h2 className="text-xl font-black italic text-slate-900 uppercase tracking-tighter">Historial</h2><button onClick={onBack} className="bg-slate-100 p-2 rounded-xl text-xs font-bold text-slate-500 transition-colors hover:bg-slate-200">Volver</button></div>
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {hist.length === 0 ? <p className="text-center py-20 opacity-30 uppercase text-xs font-black text-slate-900">Historial vacío</p> : hist.map((o: any) => (
                    <div key={o.id} className="bg-white p-5 rounded-3xl shadow-sm border border-slate-100 text-slate-900">
                        ...
                        {o.signatureClient && (<div className="mt-2 border-t border-slate-100 pt-2"><p className="text-[8px] font-black text-slate-300 uppercase mb-1 tracking-widest">Firma Receptor</p><img src={o.signatureClient} alt="Sig" className="h-8 object-contain opacity-60" /></div>)}
                    </div>
                ))}
            </div>
        </div>
...
function ReturnPumpsForm({ drivers, pumps, onFinish, pharmacyId }: any) {
    ...
    return (
        <div className="space-y-6 p-4 animate-in zoom-in-95 text-slate-900 text-slate-900">
            ...
            <div className="bg-white p-6 rounded-3xl shadow-lg border border-slate-100 space-y-6 text-slate-900">
                ...
                {driverId && driverPumps.map((p: any) => (<div key={p.id} onClick={() => setSelectedPumps((s: string[]) => s.includes(p.id) ? s.filter((i: any)=>i!==p.id) : [...s, p.id])} className={`flex justify-between items-center p-4 rounded-xl border-2 transition-all cursor-pointer ${selectedPumps.includes(p.id) ? 'border-emerald-500 bg-emerald-50' : 'border-slate-100'}`}><div><p className="font-black text-slate-900 text-sm text-slate-900">S/N: {p.code}</p></div>{selectedPumps.includes(p.id) && <CheckCircle className="w-5 h-5 text-emerald-500" />}</div>))}
            ...
            <button onClick={processReturn} disabled={loading || !signDriver || !signStaff || selectedPumps.length === 0} className="w-full bg-[#10b981] text-slate-900 py-5 rounded-3xl font-black uppercase text-xs active:scale-95 disabled:opacity-50 shadow-xl transition-all">Liberar Deuda del Chofer</button>
        </div>
    );
}

/* ==========================================================================
   5. VISTAS DE PANTALLA (DEFINIDAS ANTES QUE APP)
   ========================================================================== */
...
function PharmacyAdminView({ orders, pumps, clients, staff, onLogout, user, pharmacyId, allPumps, drivers }: any) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col relative text-slate-900 text-slate-900">
        <header className="p-8 pt-12 bg-white border-b border-slate-100 flex justify-between items-end shadow-sm"><div><h2 className="text-2xl font-black italic text-slate-900 uppercase tracking-tighter">{user.city}</h2><p className="text-[10px] font-black text-emerald-600 mt-1 uppercase tracking-widest">{user.role.replace('_', ' ')}</p></div>{section!=='menu'?<button onClick={()=>setSection('menu')} className="p-4 bg-slate-100 rounded-2xl transition-colors hover:bg-slate-200 text-slate-900"><ChevronLeft className="text-slate-900"/></button>:<button onClick={onLogout} className="p-4 bg-red-50 text-red-500 rounded-2xl transition-colors active:bg-red-100"><LogOut/></button>}</header>
        <div className="flex-1 overflow-y-auto p-6 pb-24 text-slate-900">
            {section==='menu' && (
              <div className="grid grid-cols-2 gap-4 animate-in fade-in text-slate-900">
                ...
                <MenuCard icon={<Users/>} label="Clientes" color="bg-slate-800" onClick={()=>setSection('list_clients')}/>
                <MenuCard icon={<ShieldCheck/>} label="Personal" color="bg-amber-500" onClick={()=>setSection('staff')}/>
...
function SelectionView({ onLogin, onRegister }: { onLogin: () => void, onRegister: () => void }) {
  return (
    <div className="min-h-screen bg-[#0f172a] flex flex-col items-center justify-center p-8 text-white relative overflow-hidden text-center text-center">
        ...
        <div className="w-full max-w-xs space-y-4">
            <button onClick={onLogin} className="w-full bg-white text-slate-900 py-6 rounded-3xl font-black uppercase text-xs flex items-center justify-between px-10 shadow-xl transition-all active:scale-95"><span>Acceso Portal</span><ArrowRight className="w-5 h-5 text-emerald-600"/></button>
            <button onClick={onRegister} className="w-full bg-white/5 border border-white/10 py-6 rounded-3xl font-black uppercase text-xs flex items-center justify-between px-10 hover:bg-white/10 transition-all active:scale-95 text-white"><span>Registrarse</span><Plus className="w-5 h-5 text-emerald-400"/></button>
        </div>
...
function RegisterView({ onBack }: any) {
    ...
    if (newPin) return (<div className="min-h-screen bg-[#0f172a] flex flex-col items-center justify-center p-8 text-white text-center shadow-inner"><h2 className="text-3xl font-black italic mb-4 uppercase text-white tracking-tighter">Registro Exitoso</h2><p className="text-sm text-slate-400 mb-2 uppercase tracking-widest">Tu PIN de acceso es:</p><p className="text-7xl font-black text-emerald-400 mb-10 tracking-tighter text-emerald-400">{newPin}</p><button onClick={onBack} className="w-full bg-white text-slate-900 py-5 rounded-3xl font-black uppercase text-xs active:scale-95 transition-all text-slate-900">Ir al Login</button></div>);
    return (
        <div className="min-h-screen bg-[#0f172a] flex flex-col p-8 text-white overflow-y-auto pb-20">
            <button onClick={onBack} className="self-start p-4 bg-white/5 rounded-2xl mb-8 transition-colors hover:bg-white/10 text-white"><ChevronLeft/></button>
            <h2 className="text-3xl font-black italic mb-8 uppercase tracking-tighter text-white text-white">Crear Cuenta</h2>
            ...
            <div className="space-y-4">
                ...
                <input className="w-full bg-white/5 p-4 rounded-2xl font-bold border border-white/10 outline-none text-white placeholder-white/30 text-white" placeholder="Nombre completo" onChange={e=>setForm({...form, name: e.target.value})} />
                <input className="w-full bg-white/5 p-4 rounded-2xl font-bold border border-white/10 outline-none text-white placeholder-white/30 text-white" placeholder="Email" onChange={e=>setForm({...form, email: e.target.value})} />
                <div className="grid grid-cols-2 gap-2 text-slate-900">
                    <select className="bg-white/5 p-4 rounded-2xl border border-white/10 text-sm outline-none text-white text-white" onChange={e=>setForm({...form, state: e.target.value})}><option value="" className="text-black">Estado...</option>{LOCATIONS['EC']?.states.map((s: any)=><option key={s} value={s} className="text-black text-black">{s}</option>)}</select>
                    <input className="bg-white/5 p-4 rounded-2xl border border-white/10 font-bold outline-none text-sm text-white placeholder-white/30 text-white" placeholder="Ciudad" onChange={e=>setForm({...form, city: e.target.value})} />
                </div>
                <SignaturePad onSave={setSign} label="Tu Firma (Validación ID)" />
                <button onClick={register} disabled={load || !sign} className="w-full bg-emerald-600 py-6 rounded-3xl font-black uppercase text-sm mt-4 shadow-xl active:scale-95 transition-all text-slate-900 shadow-emerald-500/20">{load ? "Procesando..." : "Completar Registro"}</button>
            </div>
        </div>
    );
}
...
function SuperAdminView({ onLogout, employees, drivers }: any) {
    ...
    return (<div className="min-h-screen bg-[#0f172a] text-white p-8 max-w-md mx-auto flex flex-col text-white text-white text-white"><header className="flex justify-between items-end mb-10 text-white text-white"><div><h2 className="text-3xl font-black italic tracking-tighter uppercase text-white">Master Hub</h2><p className="text-[10px] text-emerald-500 font-bold uppercase tracking-widest opacity-60 text-white text-white">Global Licenses</p></div><button onClick={onLogout} className="p-4 bg-white/10 rounded-2xl transition-colors hover:bg-white/20 text-white text-white"><LogOut/></button></header><div className="space-y-6 flex-1 overflow-y-auto text-white text-white text-white"><div className="bg-white/5 p-8 rounded-3xl border border-white/10 space-y-4 shadow-2xl text-white"><h3 className="font-bold flex gap-2 text-white text-white text-sm uppercase tracking-widest"><Ticket className="text-emerald-500"/> Generate License</h3><input className="w-full bg-slate-800 p-4 rounded-2xl outline-none text-sm border border-white/5 font-bold text-white uppercase placeholder-white/20 text-white text-white" placeholder="Email del cliente" value={e} onChange={v=>setE(v.target.value)}/><button onClick={gen} className="w-full bg-emerald-600 text-slate-900 py-5 rounded-3xl font-black uppercase text-xs active:scale-95 shadow-lg shadow-emerald-500/20 text-slate-900 text-slate-900">Generar Código</button></div><div className="space-y-3">{l.map((x: any)=>(<div key={x.id} className="bg-white/5 p-5 rounded-3xl flex justify-between border border-white/5 items-center text-white text-white text-white"><div><p className="font-black text-emerald-400 tracking-wider text-lg">{x.code}</p><p className="text-[10px] opacity-40 uppercase tracking-widest text-white text-white text-white">{x.assignedEmail}</p></div><span className={`text-[8px] px-3 py-1 rounded-full uppercase font-black ${x.status === 'active' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'}`}>{x.status}</span></div>))}</div></div></div>);
}
...
function DriverWorkflow({ order, allPumps, user }: any) {
    ...
    return (
      <div className="flex-1 flex flex-col bg-white text-slate-900">
        <div className="bg-[#0f172a] p-10 pt-16 rounded-b-3xl text-white shadow-2xl">
        ...
        <div className="flex-1 p-8 space-y-8 overflow-y-auto text-slate-900">
          {step === 'pickup' ? (
            <div className="space-y-6">
              ...
              <div className="bg-slate-50 p-6 rounded-3xl shadow-inner text-slate-900"><p className="text-[10px] font-black text-slate-400 mb-4 uppercase tracking-widest text-slate-900">Equipos para Recoger:</p>{order.pumps.map((p:any)=><p key={p.code} className="font-black text-2xl text-[#0f172a] mb-1 text-slate-900">S/N: {p.code}</p>)}</div>
              <SignaturePad onSave={setS1} label="Firma Conformidad Chofer" />
            </div>
          ) : (
            <div className="space-y-6">
              <div className="p-8 bg-[#0f172a] rounded-3xl text-white shadow-xl text-white">
        ...
        <div className="p-6 border-t bg-white text-slate-900"><button onClick={action} disabled={!s1 || (step==='delivery'&&!s2)} className="w-full bg-[#10b981] text-slate-900 py-6 rounded-3xl font-black uppercase text-sm shadow-xl active:scale-95 transition-all text-slate-900">Confirmar Paso</button></div>
      </div>
    );
}

function DriverApp({ orders, allPumps, user, onLogout, myCustodyPumps, allOrders }: any) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col text-slate-900 text-slate-900">
        ...
        {myCustodyPumps.length > 0 && (<div className="mx-6 mt-4 bg-red-500 p-5 rounded-3xl text-white shadow-lg animate-pulse text-white text-white"><div className="flex items-center gap-2 mb-2 text-white text-white text-white"><AlertTriangle/><span className="font-black text-xs uppercase tracking-widest text-white text-white text-white">DEVOLUCIÓN REQUERIDA</span></div><div className="flex flex-wrap gap-2 text-white text-white text-white text-white">{myCustodyPumps.map((p:any) => <span key={p.id} className="text-[9px] bg-white text-red-600 px-2 py-1 rounded font-black text-red-600 text-red-600">{p.code}</span>)}</div></div>)}
        <div className="p-6 space-y-4 flex-1 overflow-y-auto pb-20 text-slate-900 text-slate-900 text-slate-900">{orders.filter((o:any)=>o.status==='ready').length === 0 ? <div className="text-center py-20 opacity-30 font-black text-xs uppercase tracking-widest text-slate-900 text-slate-900 text-slate-900 text-slate-900">Sin rutas disponibles</div> : orders.filter((o:any)=>o.status==='ready').map((o: any) => (<div key={o.id} className="bg-white p-8 rounded-3xl shadow-xl border border-slate-100 animate-in slide-in-from-bottom-8 duration-500 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900"><div className="flex justify-between mb-6 text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900"><span className="text-[10px] font-black text-indigo-600 tracking-widest uppercase text-indigo-600 text-indigo-600">#{o.orderCode}</span><span className="bg-slate-100 text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-tighter text-slate-600 text-slate-600">{o.status}</span></div><h3 className="text-2xl font-black text-slate-800 mb-2 leading-none text-slate-900 text-slate-900 text-slate-900 text-slate-900 text-slate-900">{o.clientName}</h3><p className="text-xs text-slate-500 mb-8 flex items-center gap-2 font-bold uppercase text-slate-400 text-slate-400 text-slate-400 text-slate-400"><MapPin className="w-4 h-4 text-emerald-500"/> {o.address}</p><button onClick={()=>claimOrder(o.id)} className="w-full bg-[#0f172a] text-white py-5 rounded-2xl font-black uppercase text-xs active:scale-95 transition-all shadow-xl shadow-xl shadow-xl shadow-xl shadow-xl shadow-xl">Aceptar Ruta</button></div>))}</div>
    </div>
  );
}

/* ==========================================================================
   7. COMPONENTE APP (ROOT)
   ========================================================================== */
...
export default function App() {
  ...
  if (view === 'super_admin') return <SuperAdminView onLogout={() => { setActiveEmployee(null); setView('selection'); }} employees={employees} drivers={drivers} />;
